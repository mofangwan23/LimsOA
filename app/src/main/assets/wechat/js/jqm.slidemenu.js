$(document).on("pageinit", function(e){
    ...
    $(document).on("tap click swipeleft",".ui-page-active", function(e){
        if(sm.data('slideopen'))
        {
        slidemenu(sm, sm.data('slideopen'));
     	$(":jqmData(role='listview')").find("a").removeClass("ui-btn-active"); 
    ...
    $(document).on("scrollstart",".ui-page-active", function(e){
    	 if(sm.data('slideopen')){
        e.stopImmediatePropagation();
		e.preventDefault();
    	 }
...
function slidemenu(sm, only_close) {
	...
	if (!sm.data('slideopen') && !only_close) {
		sm.show().animate({width: '220px', avoidTransforms: false, useTranslate3d: true}, 'fast');
		//$(".ui-page-active").css('left', '220px');
