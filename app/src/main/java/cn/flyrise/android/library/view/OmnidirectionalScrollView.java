...
/**
 * <b>类功能描述：</b><div style="margin-left:40px;margin-top:-10px"> 具有弹性效果的全方向ScrollView,参考ScrollView与HorizontalScrollView源码 (惯性回弹待做...) </div>
 * @author <a href="mailto:184618345@qq.com">017</a>
 * @version 1.0 </p> 修改时间：</br> 修改备注：</br>
 */
public class OmnidirectionalScrollView extends FrameLayout {
	 ...
	 * ScrollView knows that it does not have to do anything.
	 */
	...
	/**
	 * True if the user is currently dragging this ScrollView around. This is not the same as 'is being flinged', which can be checked by
	...
	public OmnidirectionalScrollView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initScrollView();
	}
	...
	private boolean inChild(int x, int y) {
		if (getChildCount() > 0) {
			...
			final View child = getChildAt(0);
			return !(y < child.getTop() - scrollY || y >= child.getBottom() - scrollY || x < child.getLeft() - scrollX
		...
		switch (action & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_MOVE:
				if (mIsBeingDragged || scrollableOutsideTouch) {
					...
					final float y = ev.getY();// getY(activePointerIndex)
					final int deltaY = (int) (mLastMotionY - y);
					mLastMotionY = y;
					final float x = ev.getX();// getX(activePointerIndex)
					...
					if (isNeedMove() && flexible) {
						...
						// 移动布局
						inner.layout(inner.getLeft() - deltaX / 2, inner.getTop() - deltaY / 2, inner.getRight() - deltaX / 2,
			...
			case MotionEvent.ACTION_UP:
				if (mIsBeingDragged || scrollableOutsideTouch) {
					if (mFlingEnabled) {
						...
						final int initialVelocitx = (int) velocityTracker.getXVelocity(mActivePointerId);
			...
			if (top < viewBottom && viewTop < bottom) {
				...
				if (focusCandidate == null) {
					...
					final boolean viewIsCloserToBoundary =
							(topFocus && viewTop < focusCandidate.getTop()) || (!topFocus && viewBottom > focusCandidate.getBottom());
		...
		for (int i = 0; i < count; i++) {
			...
			if (left < viewRight && viewLeft < right) {
				...
				if (focusCandidate == null) {
					...
					final boolean viewIsCloserToBoundary =
							(leftFocus && viewLeft < focusCandidate.getLeft()) || (!leftFocus && viewRight > focusCandidate.getRight());
					if (foundFullyContainedFocusable) {
						if (viewIsFullyContained && viewIsCloserToBoundary) {
                            /*
                             * We're dealing with only fully contained views, so it has to be closer to the boundary to beat our candidate
	 ...
	 * @param direction the scroll direction: {@link View#FOCUS_UP} to go the top of the view or {@link View#FOCUS_DOWN} to go the bottom
	 ...
	 * scrollview. </p>
	 * @param direction the scroll direction: {@link View#FOCUS_UP} to go upward {@link View#FOCUS_DOWN} to downward
	...
	private boolean scrollAndFocusV(int direction, int top, int bottom) {
		...
		final boolean up = direction == View.FOCUS_UP;
		...
		if (top >= containerTop && bottom <= containerBottom) {
			...
			final int delta = up ? (top - containerTop) : (bottom - containerBottom);
			doScrollY(delta);
		}
	...
	private boolean scrollAndFocusH(int direction, int left, int right) {
		...
		if (left >= containerLeft && right <= containerRight) {
			...
			final int delta = goLeft ? (left - containerLeft) : (right - containerRight);
			doScrollX(delta);
		}
		if (newFocused != findFocus() && newFocused.requestFocus(direction)) {
			mScrollViewMovedFocus = true;
	...
	public boolean arrowScrollV(int direction) {
		...
		if (nextFocused != null && isWithinDeltaOfScreenV(nextFocused, maxJump, getHeight())) {
			...
			final int scrollDelta = computeScrollDeltaToGetChildRectOnScreenV(mTempRect);
	...
	public boolean arrowScrollH(int direction) {
		...
		if (nextFocused != null && isWithinDeltaOfScreenH(nextFocused, maxJump)) {
			nextFocused.getDrawingRect(mTempRect);
			offsetDescendantRectToMyCoords(nextFocused, mTempRect);
			final int scrollDelta = computeScrollDeltaToGetChildRectOnScreenH(mTempRect);
	 ...
	 * @param x the commonGroup where to scroll on the X axis
	 * @param y the commonGroup where to scroll on the Y axis
	 */
	 ...
	 * @param child the IView to scroll to
	 */
	private void scrollToChild(View child) {
		child.getDrawingRect(mTempRect);
        /* Offset from child's local coordinates to ScrollView coordinates */
		offsetDescendantRectToMyCoords(child, mTempRect);
		final int scrollDeltaV = computeScrollDeltaToGetChildRectOnScreenV(mTempRect);
		final int scrollDeltaH = computeScrollDeltaToGetChildRectOnScreenH(mTempRect);
	 ...
	 * off screen.
	 * <p/>
	 * This is more expensive than the default {@link ViewGroup} implementation, otherwise this behavior might have been made the default.
	 */
	@Override
	 ...
	 * @param velocityY The initial velocity in the Y direction. Positive numbers mean that the finger/cursor is moving down the screen,
	...
	public void fling(int velocityX, int velocityY) {
		if (getChildCount() > 0) {
			...
			// && newFocused.requestFocus(movingDown ? IView.FOCUS_DOWN :
			// IView.FOCUS_UP)) {
			// mScrollViewMovedFocus = true;
			// mScrollViewMovedFocus = false;
			// }
	...
	public void scrollTo(int x, int y) {
		// we rely on the fact the IView.scrollBy calls scrollTo.
		if (getChildCount() > 0) {
			final View child = getChildAt(0);
			x = clamp(x, getWidth() - getPaddingRight() - getPaddingLeft(), child.getWidth());
			y = clamp(y, getHeight() - getPaddingBottom() - getPaddingTop(), child.getHeight());
	...
	private int clamp(int n, int my, int child) {
		if (my >= child || n < 0) {
            /*
             * my >= child is this case: |--------------- me ---------------| |------ child ------| or |--------------- me ---------------| |------ child ------| or |--------------- me ---------------| |------ child ------| n < 0 is this case: |------ me ------| |-------- child --------| |-- mScrollX --|
             */
			return 0;
		}
		if ((my + n) > child) {
	...
	public void animation() {
		// 开启移动动画
		final TranslateAnimation ta = new TranslateAnimation(0, -inner.getLeft(), 0, -inner.getTop());
		ta.setDuration(200);
	...
	public boolean isNeedMove() {
		...
		final int offsetX = inner.getMeasuredWidth() - getWidth();
		final int scrollX = getScrollX();
		if (scrollX == 0 || scrollX == offsetX) {
			return true;
		}
		final int offsetY = inner.getMeasuredHeight() - getHeight();
		final int scrollY = getScrollY();
