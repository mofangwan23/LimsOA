...
//    }
     ...
     * @return
     */
    public DPInfo[][] obtainDPInfo(int year, int month, int year2, int month2, int day2, boolean useCache) {
        HashMap<Integer, DPInfo[][]> dataOfYear = DATE_CACHE.get(year);
        if (null != dataOfYear && dataOfYear.size() != 0 && useCache) {
            DPInfo[][] dataOfMonth = dataOfYear.get(month);
            if (dataOfMonth != null) {
                return dataOfMonth;
            }
            dataOfMonth = buildDPInfo(year, month, year2, month2, day2);
            dataOfYear.put(month, dataOfMonth);
            return dataOfMonth;
        }
        if (null == dataOfYear) dataOfYear = new HashMap<>();
        DPInfo[][] dataOfMonth = buildDPInfo(year, month, year2, month2, day2);
        dataOfYear.put((month), dataOfMonth);
        DATE_CACHE.put(year, dataOfYear);
        return dataOfMonth;
    }
     ...
     * @return
     */
    private DPInfo[][] buildDPInfo(int year, int month, int year2, int month2, int day2) {
        DPInfo[][] info = new DPInfo[6][7];
        ...
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                tmp.strG = strG[i][j];
                // 是否是上个月的日期
                if (i == 0 && Integer.parseInt(tmp.strG) > 7) {
                    tmp.isLastDate = true;
                }
                // 是否是下个月的日期
//                int index = 0;
...
//                    index = 5;
//                }
//                if (i == index && Integer.parseInt(tmp.strG) < 17) {
//                    tmp.isNextDate = true;
//                }
                if (i >= 4 && Integer.parseInt(tmp.strG) < 17) {
                    tmp.isNextDate = true;
                }
                ...
                if (!TextUtils.isEmpty(tmp.strG) && strHoliday.contains(tmp.strG))
                    tmp.isHoliday = true;
                if (!TextUtils.isEmpty(tmp.strG)) tmp.isToday =
                        c.isToday(year, month, Integer.valueOf(tmp.strG), year2, month2, day2);
                if (strWeekend.contains(tmp.strG)) tmp.isWeekend = true;
                if (c instanceof DPCNCalendar) {
                    if (!TextUtils.isEmpty(tmp.strG)) tmp.isSolarTerms =
                            ((DPCNCalendar) c).isSolarTerm(year, month, Integer.valueOf(tmp.strG));
