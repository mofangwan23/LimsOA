...
public abstract class DPCalendar {
     ...
     * @param month1
     * @param day1
     * @param year2 设置那天所在的年
     * @param month2 设置那天所在的月
     * @param day2 设置那天所在的日
     * @return
     */
    public boolean isToday(int year1, int month1, int day1,int year2, int month2, int day2) {
        Calendar c1 = Calendar.getInstance();
        Calendar c2 = Calendar.getInstance();
    ...
    public String[][] buildMonthG(int year, int month) {
        ...
        int lastMonth = month - 1; //上个月是几月
        int lastMonthOfYear = year; // 上个月是在第几个年份里
        if (lastMonth == 0) {
            lastMonth = 12;
            lastMonthOfYear -= 1;
        }
...
//            }
            for (int j = 0; j < 7; j++) {
                ...
                if (i == 0 && j >= dayOfWeek) {
                    ...
                    tmp[i][j] = "" + day;
                    day++;
                } else if (i == 0 && j <= dayOfWeek) {
                    // 上个月的日期
                    tmp[i][j] = dayInlastMonth - dayOfWeek + 1 + j+ "";
                }
                else if (i > 0 && day > daysInMonth){
                    // 下个月的日期
                    tmp[i][j] = ++nextDay + "";
