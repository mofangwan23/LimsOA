...
public class TiledBitmapCanvas implements CanvasLite {
	...
	public static final int MAX_VERSIONS = 2;  //本来是10 改成2减少内存占用

	private int mTileSize = DEFAULT_TILE_SIZE;

	//    private List<Bitmap> verBitmap=new ArrayList<Bitmap>();
//    private List<Bitmap> toBitmap=new ArrayList<Bitmap>();
	public class Tile {
...
//                	verBitmap.add(this.bitmap);
		...
		public Tile(int x, int y, int version) {
			this.x = x;
			this.y = y;
			versions = new ArrayList<>(MAX_VERSIONS);
			bottom = version;
...
//        }
	...
	private void load(Bitmap bitmap) {
		...
		for (int j = 0; j < mTilesY; j++) {
			for (int i = 0; i < mTilesX; i++) {
				...
				final Tile t = new Tile(i, j, mNewVersion);
				mTiles[p] = t;
				if (bitmap != null) {
	...
	public void drawTo(Canvas drawCanvas, float left, float top, Paint paint, boolean onlyDirty) {
		src = new Rect(0, 0, mTileSize, mTileSize);
		dst = new Rect(0, 0, mTileSize, mTileSize);
		if (DEBUG) {
