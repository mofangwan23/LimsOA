...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	/**
	 * Gets whether an indicator graphic should be displayed when the IView is in
	 * a state where a Pull-to-Refresh can happen. An example of this state is
	 * when the Adapter IView is scrolled to the top and the mode is set to
	...
	/**
	 * Sets the Empty IView to be used by the Adapter IView.
	 * <p/>
	 * We need it handle it ourselves so that we can Pull-to-Refresh when the
	 * Empty IView is shown.
	 * <p/>
	 * Please note, you do <strong>not</strong> usually need to call this method
	 * yourself. Calling setEmptyView on the AdapterView will automatically call
	 * this method and set everything up. This includes when the Android
	 * Framework automatically sets the Empty IView based on it's ID.
	 * 
	 * @param newEmptyView - Empty IView to be used
	 */
	public final void setEmptyView(View newEmptyView) {
		FrameLayout refreshableViewWrapper = getRefreshableViewWrapper();

		// If we already have an Empty IView, remove it
		if (null != mEmptyView) {
	...
	/**
	 * Sets whether an indicator graphic should be displayed when the IView is in
	 * a state where a Pull-to-Refresh can happen. An example of this state is
	 * when the Adapter IView is scrolled to the top and the mode is set to
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			// If we can't pull down, but have a IView then remove it
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			if (DEBUG) {
				Log.d(LOG_TAG, "isFirstItemVisible. Empty IView.");
			}
			return true;
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			if (DEBUG) {
				Log.d(LOG_TAG, "isLastItemVisible. Empty IView.");
			}
			return true;
		} else {
