...
public class Particle {
    ...
    private static final int DEFAULT_LIFETIME = 100; // play with this
    ...
    private static int lifetime = 150; // particle dies when it reaches this value
    ...
    Particle(int x, int y) {
        ...
        this.age = 0;
        this.xv = (rndDbl(MAX_SPEED * 2) - MAX_SPEED);
        this.yv = (rndDbl(MAX_SPEED * 2) - MAX_SPEED);
    ...
    private static double rndDbl(double max) {
        return (double) 0 + (max - (double) 0) * Math.random();
    }
...
//    }
    ...
    public void update() {
        if (this.state != STATE_DEAD) {
            ...
            if (this.age >= lifetime) { // reached the end if its life
                this.state = STATE_DEAD;
            }
