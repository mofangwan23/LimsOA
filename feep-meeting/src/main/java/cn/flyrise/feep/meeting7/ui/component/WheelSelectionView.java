				...
				if (initialY - newY == 0) { // stopped
					...
					if (remainder == 0) {
						...
						if (remainder > itemHeight / 2) {
							WheelSelectionView.this.post(new Runnable() {
								@Override
								public void run() {
...
//				itemView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
				itemView.setTextColor(MeetingToolkitKt.getNormalTextColor());
			}
			else {
//				itemView.getPaint().setFakeBoldText(false);
//				itemView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
				itemView.setTextColor(MeetingToolkitKt.getUnableTextColor());
			}
		}
	}
	...
	public void setSeletion(int position) {
		...
		this.post(new Runnable() {
			@Override public void run() {
				WheelSelectionView.this.smoothScrollTo(0, p * itemHeight);
			}
		});
	}
