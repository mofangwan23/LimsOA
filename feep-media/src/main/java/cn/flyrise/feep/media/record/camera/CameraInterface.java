...
import cn.flyrise.feep.media.record.camera.util.AngleUtil;
import cn.flyrise.feep.media.record.camera.util.CameraParamUtil;
import cn.flyrise.feep.media.record.camera.util.CheckPermission;
import cn.flyrise.feep.media.record.camera.util.DeviceUtil;
import cn.flyrise.feep.media.record.camera.util.FileUtil;
import cn.flyrise.feep.media.record.camera.util.ScreenUtils;
...
public class CameraInterface implements Camera.PreviewCallback {
	...
	@SuppressLint("StaticFieldLeak") private volatile static CameraInterface mCameraInterface;
	...
	public static synchronized CameraInterface getInstance() {
		if (mCameraInterface == null)
			synchronized (CameraInterface.class) {
				if (mCameraInterface == null) mCameraInterface = new CameraInterface();
			}
	...
	public void setZoom(float zoom, int type) {
		...
		if (mParams == null) mParams = mCamera.getParameters();
		if (!mParams.isZoomSupported() || !mParams.isSmoothZoomSupported()) return;
		switch (type) {
			case TYPE_RECORDER:
				//如果不是录制视频中，上滑不会缩放
			...
			case TYPE_CAPTURE:
				if (isRecorder) return;
				int scaleRate = (int) (zoom / 50);//每移动50个像素缩放一个级别
	...
	void doOpenCamera(CameraOpenOverCallback callback) {
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
			if (!CheckPermission.isCameraUseable(SELECTED_CAMERA)) return;
		}
		if (mCamera == null) openCamera(SELECTED_CAMERA);
	...
	public synchronized void switchCamera(SurfaceHolder holder, float screenProp) {
		if (SELECTED_CAMERA == CAMERA_POST_POSITION) SELECTED_CAMERA = CAMERA_FRONT_POSITION;
		else SELECTED_CAMERA = CAMERA_POST_POSITION;
		doDestroyCamera();
	...
	public void doStartPreview(SurfaceHolder holder, float screenProp) {
		if (this.screenProp < 0) this.screenProp = screenProp;
	...
	public void doStopPreview() {//停止预览
		if (null != mCamera) {
			try {
	...
	void doDestroyCamera() {//销毁Camera
		if (null == mCamera) return;
		try {
			mCamera.setPreviewCallback(null);
			mSwitchView = null;
			mFlashLamp = null;
			mCamera.stopPreview();//这句要在stopPreview后执行，不然会卡顿或者花屏
	...
	public void startRecord(Surface surface, float screenProp) {
		...
		Camera.Parameters parameters = mCamera.getParameters();//获取第一帧图片
		...
		Matrix matrix = new Matrix();
		if (SELECTED_CAMERA == CAMERA_POST_POSITION) matrix.setRotate(nowAngle);
		else if (SELECTED_CAMERA == CAMERA_FRONT_POSITION) matrix.setRotate(270);
		...
		if (videoSize.width == videoSize.height) mediaRecorder.setVideoSize(preview_width, preview_height);
		else mediaRecorder.setVideoSize(videoSize.width, videoSize.height);

		if (SELECTED_CAMERA == CAMERA_FRONT_POSITION) {//手机预览倒立的处理
			if (cameraAngle == 270) {//横屏
				if (nowAngle == 0) mediaRecorder.setOrientationHint(180);
				else if (nowAngle == 270) mediaRecorder.setOrientationHint(270);
				else mediaRecorder.setOrientationHint(90);
			}
			else {
				if (nowAngle == 90) mediaRecorder.setOrientationHint(270);
				else if (nowAngle == 270) mediaRecorder.setOrientationHint(90);
				else mediaRecorder.setOrientationHint(nowAngle);
			}
		}
		else mediaRecorder.setOrientationHint(nowAngle);

		int mediaQuality = JCameraView.MEDIA_QUALITY_MIDDLE;
		if (DeviceUtil.isHuaWeiRongyao()) mediaRecorder.setVideoEncodingBitRate(4 * 100000);
		else mediaRecorder.setVideoEncodingBitRate(mediaQuality);
		mediaRecorder.setPreviewDisplay(surface);

		videoFileName = "video_" + System.currentTimeMillis() + ".mp4";
		if (saveVideoPath.equals("")) saveVideoPath = Environment.getExternalStorageDirectory().getPath();
	...
	public void stopRecord(boolean isShort, StopRecordCallback callback) {//停止录像
		if (!isRecorder) return;
		if (mediaRecorder != null) {
			...
			if (isShort) {
				if (FileUtil.deleteFile(videoFileAbsPath)) callback.recordResult(null, null);
				return;
			}
			doStopPreview();
