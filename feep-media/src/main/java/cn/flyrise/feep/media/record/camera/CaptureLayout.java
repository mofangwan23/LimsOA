...
import cn.flyrise.feep.media.record.camera.listener.CaptureListener;
import cn.flyrise.feep.media.record.camera.listener.ClickListener;
import cn.flyrise.feep.media.record.camera.listener.TypeListener;
...
public class CaptureLayout extends FrameLayout {
	...
	public CaptureLayout(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		else layout_width = outMetrics.widthPixels / 2;
	...
	public void initEvent() {
		iv_custom_right.setVisibility(GONE);//默认Typebutton为隐藏
		btn_cancel.setVisibility(GONE);
		btn_confirm.setVisibility(GONE);
	}

	public void startTypeBtnAnimator() {//拍照录制结果后的动画
		if (this.iconLeft != 0) iv_custom_left.setVisibility(GONE);
		else btn_return.setVisibility(GONE);
		if (this.iconRight != 0) iv_custom_right.setVisibility(GONE);
	...
	@SuppressLint("RtlHardcoded") private void initView() {
		setWillNotDraw(false);
		btn_capture = new CaptureButton(getContext(), button_size);//拍照按钮
		...
		btn_capture.setCaptureLisenter(new CaptureListener() {
			...
			public void takePictures() {
				if (captureLisenter != null) captureLisenter.takePictures();
			}

			@Override
			public void recordShort(long time) {
				if (captureLisenter != null) captureLisenter.recordShort(time);
				startAlphaAnimation();
			}
			...
			public void recordEnd(long time) {
				if (captureLisenter != null) captureLisenter.recordEnd(time);
				startAlphaAnimation();
			...
			public void recordZoom(float zoom) {
				if (captureLisenter != null) captureLisenter.recordZoom(zoom);
			}

			@Override
			public void recordError() {
				if (captureLisenter != null) captureLisenter.recordError();
			}
		});
		//取消按钮
		...
		btn_cancel.setOnClickListener(view -> {
			if (typeLisenter != null) typeLisenter.cancel();
			startAlphaAnimation();
		});
		btn_confirm = new TypeButton(getContext(), TypeButton.TYPE_CONFIRM, button_size);//确认按钮
		...
		btn_confirm.setOnClickListener(view -> {
			if (typeLisenter != null) typeLisenter.confirm();
			startAlphaAnimation();
		});
		btn_return = new ReturnButton(getContext(), (int) (button_size / 2.5f));//返回按钮
		...
		btn_return.setOnClickListener(v -> {
			if (leftClickListener != null) leftClickListener.onClick();
		});
		iv_custom_left = new ImageView(getContext());//左边自定义按钮
		...
		iv_custom_right = new ImageView(getContext());//右边自定义按钮
		...
		iv_custom_right.setOnClickListener(v -> {
			if (rightClickListener != null) rightClickListener.onClick();
		});
	...
	public void resetCaptureLayout() {
		...
		if (this.iconLeft != 0) iv_custom_left.setVisibility(VISIBLE);
		else btn_return.setVisibility(VISIBLE);

		if (this.iconRight != 0) iv_custom_right.setVisibility(VISIBLE);
	}
	...
	public void setButtonFeatures(int state) {
		if (btn_capture != null) btn_capture.setButtonFeatures(state);
		if (txt_tip == null) return;
