...
public class JCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
	...
	public static final int MEDIA_QUALITY_MIDDLE = 16 * 100000;//录制视频比特率
...
//    public static final int MEDIA_QUALITY_SORRY = 1 * 80000;
	...
	public JCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		duration = a.getInteger(R.styleable.JCameraView_duration_max, 10 * 1000);       //没设置默认为30s
		a.recycle();
		initData();
		initView();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (screenProp == 0) screenProp = heightSize / widthSize;
	}
	...
	public void onPause() {
		...
		CameraInterface.getInstance().isPreview();
	...
	public void setBtnState(int btnState) {
		if (mCaptureLayout != null) mCaptureLayout.setButtonFeatures(btnState);
	}
	...
	public void showPicture(Bitmap bitmap, boolean isVertical) {
		if (isVertical) mPhoto.setScaleType(ImageView.ScaleType.FIT_XY);
		else mPhoto.setScaleType(ImageView.ScaleType.FIT_CENTER);
